...
namespace Pex4Fun
{
    public class Program
    {
        ...
        {
            string folderName = submissionSrcDir.Substring(submissionSrcDir.LastIndexOf("\\") + 1);
            string srcFilePath = System.IO.Path.Combine(submissionSrcDir, fileName);
            string clusterDirectory = System.IO.Path.Combine(firstLevelResultsDir, score, folderName);
            ...
            ///string tmp = folderName.Substring(0, lastIdx);
            //string fileDirName = tmp.Substring(tmp.LastIndexOf('\\') + 1);
