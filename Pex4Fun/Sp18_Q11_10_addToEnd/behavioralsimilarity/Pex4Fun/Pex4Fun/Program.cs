...
namespace Pex4Fun
{
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            //Go from C:...codesimilarity\Pex4Fun\Sp18_Q11_10_addToEnd\behavioralsimilarity\Pex4Fun\bin\Debug\ -> C:...\codesimilarity\
                            ...
                            if (file.EndsWith(".cs"))
                            {
                                ...
                                if (FileModifier.CopyBodyToMetaProgram(file, editedMetaProjectDir, methodName, hardcodedMetaProjName))
                                {
                                    ...
                                    if (BuildDirectory.BuildSingleProject(editedMetaProjectDir, true))
                                    {
                                        ...
                                        Tuple<HashSet<string[]>, HashSet<string[]>, string> passedAndFailedTests = ExtractPexResults.ExtractTestsForEditedMetaProject(editedMetaProjectDir, fileNameMinusCS, "MetaProgram", "Check", assemblyName);
                                        ...
                                        if (passedTests.Count != 0)
                                        {
                                            ...
                                            //...Store list in file
                                            recordPassedAndFailedMethodBodies(fileNameMinusCS, passedTests, failedTests, firstLevelResultsDir,
            ...
            if (approach.ToLower() != "subsumption" && (levelToRun.ToLower() == "second" || levelToRun.ToLower() == "both"))
            {
                ...
                foreach (var pseScoreFolder in Directory.GetDirectories(firstLevelResultsDir))
                {
                    ...
                    string[] filesArr = Directory.GetFiles(pseScoreFolder);
        ...
        public static void copyOverAssemblyFile(string editedMetaProjectDir, string currDir, string assemblyFileName)
        {
            ...
            {
                Console.WriteLine("Cannot copy over updated assembly file from " + srcBinFile + " to " + destBinFile +". Currently being user by another process.");
            }
        }
